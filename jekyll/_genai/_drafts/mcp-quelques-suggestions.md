...
Difficile de parler de MCP sans parler d'agent, et des cas d'usage associés.
...
Pourquoi ? Pour contribuer au caractère déterministe et prédictible du pipeline
...
# Un marqueur de terminaison de la boucle agentique
...
aucun tool à sa disposition durant ce "tour" et sera donc contraint (autant
...
pas propice (éviter de surcharger l'infra, connaissance à priori qu'un _retry_
...
# Un marqueur de retour immédiat à l'utilisateur
...
qui retourne un grand nombre de lignes. Si l'objet de ce _tool_ est de fournir
le résultat exact de la requête à l'utilisateur (par exemple pour un export ou
une consultation directe), alors faire transiter ces lignes par le LLM n'a
aucun intérêt : au mieux il les recopie à l'identique en consommant des tokens
et en ajoutant de la latence, au pire il les tronque ou en altère le contenu.
Le marqueur permet ici de court-circuiter le LLM et de remettre le jeu de
données directement à l'utilisateur, préservant ainsi son intégrité.

Autre exemple : un _tool_ qui retourne des informations qu'on ne souhaite pas
exposer au LLM. Il peut s'agir de données sensibles (informations personnelles,
secrets, données soumises à des contraintes réglementaires) pour lesquelles on
veut limiter au maximum la surface d'exposition, ou simplement de données
volumineuses et peu pertinentes pour le raisonnement de l'agent, qui ne
feraient que parasiter sa fenêtre de contexte. Dans les deux cas, le marqueur
permet d'acheminer le résultat vers l'utilisateur sans que le LLM ne le voie
jamais passer.
...
récupérer les logs d'un serveur, utilisé par un agent pour faire du
_troubleshooting_. La fonction pouvait renvoyer plusieurs milliers de lignes,
alors que l'information recherchée pouvait se trouver dans les premières. Tout
charger à chaque appel, c'était payer le coût (en tokens, latence et saturation
de contexte) d'un volume de données dont on exploitait rarement l'intégralité.
Nous avons donc implémenté une pagination à ce _tool_ et patcher la boucle
agentique pour gérer ce cas particulier.
...
Notons que la pagination est un concept qui existe déjà dans la spécification
...
etc.). La proposition consiste donc à étendre cette fonctionnalités aux
opérations d'exécution (j'ai évoqué les _tools_, mais cela pourrait également
s'appliquer aux _resources_ par exemple).
